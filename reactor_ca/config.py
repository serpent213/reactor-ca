...
class Config:
    ...
    def create(
        ...
        elif os.environ.get(ENV_ROOT_DIR):
            resolved_root = Path(os.environ[ENV_ROOT_DIR])
        else:
            resolved_root = Path(".")
        ...
        if config_dir:
            resolved_config = Path(config_dir)
        elif os.environ.get(ENV_CONFIG_DIR):
            resolved_config = Path(os.environ[ENV_CONFIG_DIR])
        else:
            resolved_config = resolved_root / "config"

        # Resolve store directory
        resolved_store = None
        if store_dir:
            resolved_store = Path(store_dir)
        elif os.environ.get(ENV_STORE_DIR):
            resolved_store = Path(os.environ[ENV_STORE_DIR])
        else:
            resolved_store = resolved_root / "store"

        return cls(root_dir=resolved_root, config_dir=resolved_config, store_dir=resolved_store)
...
def load_config(config_path: Path | None = None) -> dict[str, Any]:
    ...
    if config_path is None:
        # Create a default Config instance and use its ca_config_path
        config_instance = Config.create()
        config_path = config_instance.ca_config_path
...
def load_hosts_config_dict(config_path: Path | None = None) -> dict[str, Any]:
    ...
    """
    if config_path is None:
        # Create a default Config instance and use its hosts_config_path
        config_instance = Config.create()
        config_path = config_instance.hosts_config_path
...
def load_hosts_config(config_path: Path) -> dict[str, HostConfig]:
    ...
    # Handle both the old format where hosts is a dict and new format where it's a list
    hosts_data = config_dict.get("hosts", {})

    # If hosts is a list of dicts with 'name' key, convert to dict keyed by name
    if isinstance(hosts_data, list):
        hosts_map = {}
        for host in hosts_data:
            if "name" in host:
                # Make a copy of the host data
                host_copy = host.copy()
                hosts_map[host["name"]] = host_copy
            else:
                console.print(f"[bold yellow]Warning:[/bold yellow] Host entry missing 'name' field: {host}")
        hosts_data = hosts_map

    # Now process the hosts dict
    try:
        for host_name, host_data in hosts_data.items():
            # Convert alternative names
            alt_names = None
            dns_names = []
            ip_addresses = []

            # Handle both direct DNS/IP lists and those nested under 'alternative_names'
            if "dns_names" in host_data:
                dns_names = host_data.get("dns_names", [])
            elif "alternative_names" in host_data and "dns" in host_data["alternative_names"]:
                dns_names = host_data["alternative_names"].get("dns", [])

            if "ip_addresses" in host_data:
                ip_addresses = host_data.get("ip_addresses", [])
            elif "alternative_names" in host_data and "ip" in host_data["alternative_names"]:
                ip_addresses = host_data["alternative_names"].get("ip", [])

            if dns_names or ip_addresses:
                alt_names = AlternativeNames(dns=dns_names, ip=ip_addresses)

            # Handle validity in different formats
            ...
            if isinstance(validity_data, dict):
                ...
                validity = ValidityConfig(days=days_valid or validity_data or 365)
            ...
            hosts_dict[host_name] = host_config
...
def save_hosts_config(hosts_dict: dict[str, HostConfig], config_path: Path) -> None:
    ...
    for host_name, host_config in hosts_dict.items():
        host_data = {
            ...
            "days_valid": host_config.validity.to_days(),
        }
        ...
        if host_config.alternative_names:
            if host_config.alternative_names.dns:
                host_data["dns_names"] = host_config.alternative_names.dns
            if host_config.alternative_names.ip:
                host_data["ip_addresses"] = host_config.alternative_names.ip
...
def create_default_ca_config() -> dict[str, Any]:
    """Create a default CA configuration.

    Returns
    -------
        Default CA configuration as a dictionary
    ...
    return {
        ...
        "email": "admin@example.com",
        "validity_days": 3650,
        "password": {"min_length": 12, "file": "", "env_var": "REACTOR_CA_PASSWORD"},
    }


def create_default_hosts_config() -> dict[str, Any]:
    """Create a default hosts configuration with an example host.

    Returns
    -------
        Default hosts configuration as a dictionary

    """
    example_host = {
        "common_name": "example.local",
        "dns_names": ["example.local", "www.example.local"],
    ...
    return {"hosts": {"example": example_host}}
