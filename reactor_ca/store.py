...
from reactor_ca.result import Failure, Result, Success, is_failure
...
def normalize_to_config(config_or_store: Config | Store | str) -> Result[Config, str]:
...
def initialize_store(store_path: str) -> Result[None, str]:
    ...
    try:
        ...
        # We already imported Config from reactor_ca.models at the top of the file
        temp_config = Config(
...
def read_ca_cert(config_or_store: Config | Store | str) -> Result[bytes, str]:
    ...
    # We can safely access value because we checked is_failure above
    config = cast(Success[Config], config_result).value
...
def write_ca_cert(config_or_store: Config | Store | str, cert_bytes: bytes) -> Result[None, str]:
...
def read_ca_key(config_or_store: Config | Store | str, password: str) -> Result[bytes, str]:
...
def write_ca_key(config_or_store: Config | Store | str, key_bytes: bytes, password: str) -> Result[None, str]:
...
def read_host_cert(config_or_store: Config | Store | str, host_id: str) -> Result[bytes, str]:
...
def write_host_cert(config_or_store: Config | Store | str, host_id: str, cert_bytes: bytes) -> Result[None, str]:
...
def read_host_key(config_or_store: Config | Store | str, host_id: str, password: str) -> Result[bytes, str]:
...
def write_host_key(
    config_or_store: Config | Store | str, host_id: str, key_bytes: bytes, password: str
) -> Result[None, str]:
...
def ca_exists(config_or_store: Config | Store | str) -> bool:
...
def host_exists(config_or_store: Config | Store | str, host_id: str) -> bool:
...
def list_hosts(config_or_store: Config | Store | str) -> Result[list[str], str]:
...
def delete_host(config_or_store: Config | Store | str, host_id: str) -> Result[None, str]:
...
def change_password(config_or_store: Config | Store | str, old_password: str, new_password: str) -> Result[None, str]:
    ...
    encrypted_key_files: list[tuple[Path, str | None]] = []
    ...
    try:
        ...
        for key_path, current_host_id in encrypted_key_files:
            # Read encrypted key (current_host_id can be None for CA key)
            with open(key_path, "rb") as f:
