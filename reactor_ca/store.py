...
def unlock_store(store: Store, password: str | None = None, ca_init: bool = False) -> Result[Store, str]:
    ...
    config_path, _store_path = resolve_paths(None, store.path)
    ...
    if ca_init:
        return Success(Store(path=store.path, password=password, unlocked=True))
...
def ca_exists(store: Store) -> bool:
    ...
    """
    store_path = Path(store.path)
    return get_ca_cert_path(store_path).exists() and get_ca_key_path(store_path).exists()
